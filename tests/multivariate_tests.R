...
call.multivariate.wrapper <- function(params) {
	...
	# Get output
	out <- NULL
	if ('ropls_modeldf_out' %in% names(params))
		out[['ropLsModelDF']] <- load.df(params[['ropls_modeldf_out']])
...
tesArgLs <- list(input_pca = c(respC = "none",
                 ...
                 example1_plsda = c(respC = "traitment",
                     ...
                     orthoI = "0",
                     testL = "FALSE",
                     .chkC = "checkEqualsNumeric(nrow(outLs[['ropLsModelDF']]), 3, tolerance = 0.5)"),
...
for(tesC in names(tesArgLs))
    tesArgLs[[tesC]] <- c(tesArgLs[[tesC]],
                          ...
                          ropls_modeldf_out = file.path(exaDirOutC, "roplsModelDF.tsv"),
